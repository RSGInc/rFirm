...
setwd("E:/Projects/Clients/FHWA/national_freight/dev")

library("RSGFAF", lib.loc="C:/Program Files/R/R-3.3.1/library")
library("data.table", lib.loc="C:/Program Files/R/R-3.3.1/library")
library("rFreight", lib.loc="~/R/win-library/3.3")
...
FAF_NUMA    <- read.csv ("./Data/corresp_taz_faf4.csv")
FIPS_NUMA   <- read.csv ("./Data/corresp_taz_fips.csv")
FAF_NUMA  <- data.table(FAF_NUMA)
FIPS_NUMA <- data.table(FIPS_NUMA)  #TAZs that have two counties inside them : (304,823,828,1432,4381,4568) ##TODO:Check this?
...
SUSB_F <- read.csv("./Data/SUSB_Firms.csv")
SUSB_F <- data.table(SUSB_F)
...
agfirms <- merge(agfirms, NAICS2007_to_NAICS2007io[,.(NAICSio, naics=as.character(NAICS))], by = "NAICSio", allow.cartesian = TRUE)
...
foreignfirms <- data.table(expand.grid(naics = as.character(unique(NAICS2007$NAICS6)), FAF4 = 801:808))
