sc_sim_pmg <- function(naics_set, ShipmentRoutesCosts){
  ...
  for(naics_run_number in 1:nrow(naics_set)){
    ...
    if (!"group" %in% names(prodc)){
      create_sample_groups(naics, groups, sprod)
    }
    ...
    for(g in 1:groups){
      ...
      startAsyncTask(
        ...
        future({ # Start the future processor
          ...
          output <- capture.output(create_inputs(naics, g, sprod))
      ...
      startAsyncTask(taskName,
                     future({
                       ...
                       runPMG(naics, g, writelog = ifelse(PMGParameters[variable=="pmglogging",value==1],TRUE,FALSE), wait = TRUE, pmgexe = file.path(SYSTEM_PMG_PATH,"pmg.exe"),
                              ...
                              log_folder = SCENARIO_OUTPUT_PATH)
        }),
        callback = function(asyncResults) {
          # asyncResults is: list(asyncTaskName,
          #                        taskResult,
          ...
          task_log_file_path <- file.path(outputdir, paste0(taskInfo$taskNaics, "_PMGRun_Log.txt"))
...
# Working on this
create_sample_groups <- function(naics, groups, sprod){
  # Assign group number to producers and consumers
  consc[naics_set[,.(NAICS,groups)],numgroups:=i.groups,on=.(InputCommodity==NAICS)]
  prodc[naics_set[,.(NAICS,groups)],numgroups:=i.groups,on="NAICS"]
...
create_inputs <- function(naics, g, sprod){
  ...
  fwrite(consc[group==g,.(InputCommodity,BuyerID,FAF4,Zone=TAZ1,NAICS,Size,OutputCommodity=NAICS,PurchaseAmountTons,PrefWeight1_UnitCost,PrefWeight2_ShipTime,SingleSourceMaxFraction)], file = file.path(SCENARIO_OUTPUT_PATH,paste0(naics, "_g", g, ".buy.csv")))
  
  conscg <- consc[group==g,.(InputCommodity,SCTG,NAICS,FAF4,Zone=TAZ1,BuyerID,Size,PurchaseAmountTons,ConVal)]
  ...
  if(sprod==1){
    ...
    fwrite(prodc[group==g,.(OutputCommodity,SellerID,FAF4,Zone=TAZ1,NAICS,Size,OutputCapacityTons,NonTransportUnitCost)], file = file.path(SCENARIO_OUTPUT_PATH,paste0(naics, "_g", g, ".sell.csv")))
    
    prodcg <- prodc[group==g,.(OutputCommodity,NAICS,SCTG,SellerID,Size,FAF4,Zone=TAZ1,OutputCapacityTons)]
    ...
    fwrite(prodc[group==g,.(OutputCommodity,SellerID,FAF4,Zone=TAZ1,NAICS,Size,OutputCapacityTons=OutputCapacityTonsG,NonTransportUnitCost)], file = file.path(SCENARIO_OUTPUT_PATH,paste0(naics, "_g", g, ".sell.csv")))
    
    prodcg <- prodc[group==g,.(OutputCommodity,NAICS,SCTG,SellerID,Size,FAF4,Zone=TAZ1,OutputCapacityTons=OutputCapacityTonsG)]
  }
  ...
  n_splits <- ceiling(npct*nShipSizet*size_per_row/(AVAILABLE_RAM/CORES_USED))
  ...
  findMinLogisticsCost <- function(split_number,modeChoiceConstants=NULL){
    ...
    if(!is.null(modeChoiceConstants)){
      df_fin <- minLogisticsCost(pc_split, modeChoiceConstants = modeChoiceConstants)
    } else {
      df_fin <- minLogisticsCost(pc_split, modeChoiceConstants=NULL)
    }
  ...
  for(iters in iter){
    if(exists("ModeChoiceConstants")){
      df_min <- rbindlist(lapply(1:n_splits,findMinLogisticsCost,ModeChoiceConstants))
    } else {
...
sample_prod_cons <- function(conscg,prodcg){
  ...
  n_splits <- ceiling(nconst*nprodt*SIZE_PER_ROW/(RAM_TO_USE/(MAX_COST_SCRIPTS)))
  ...
  sample_data <- function(split_number,fractionOfSupplierperBuyer=1.0,samplingforSeller=FALSE){
    ...
    if((pc_split[,.N]>0)){
      ...
      pc_split[,Proportion:=Proportion*((OutputCapacityTons/PurchaseAmountTons)/sum((OutputCapacityTons/PurchaseAmountTons),na.rm=TRUE)),by=.(BuyerID,SCTG,Distance_Bin)]
...
minLogisticsCost <- function(BuyerSupplierPairs, modeChoiceConstants=NULL, runmode = 0){
  ...
  for (iSCTG in unique(BuyerSupplierPairs$SCTG)){
    ...
    if(runmode==0)	{
      ...
      if(!is.null(modeChoiceConstants) & nrow(DirectPairs) > 0){
        DirectPairs <- minLogisticsCostSctgPaths(DirectPairs,iSCTG,c(1,2), modeChoiceConstants = modeChoiceConstants)
      } else if(nrow(DirectPairs) > 0) {
        DirectPairs <- minLogisticsCostSctgPaths(DirectPairs,iSCTG,c(1,2), modeChoiceConstants = NULL)
      } else {
      ...
      if(!is.null(modeChoiceConstants) & nrow(IndirectPairs) > 0){
        IndirectPairs <- minLogisticsCostSctgPaths(IndirectPairs,iSCTG,c(3:21), modeChoiceConstants=modeChoiceConstants)
      } else if (nrow(IndirectPairs) > 0) {
        IndirectPairs <- minLogisticsCostSctgPaths(IndirectPairs,iSCTG,c(3:21), modeChoiceConstants=NULL)
      } else {
      ...
      if(is.null(modeChoiceConstants) & nrow(DomesticShipmentPairs) > 0){
        DomesticShipmentPairs <- minLogisticsCostSctgPaths(DomesticShipmentPairs,iSCTG,c(4:7,15:21),modeChoiceConstants = NULL)			## include inland water
      } else if(nrow(DomesticShipmentPairs) > 0) {
        DomesticShipmentPairs <- minLogisticsCostSctgPaths(DomesticShipmentPairs,iSCTG,c(4:7,15:21),modeChoiceConstants = modeChoiceConstants)
      } else {
...
minLogisticsCostSctgPaths <- function(BuyerSupplierPairs, iSCTG, paths, modeChoiceConstants=NULL){
